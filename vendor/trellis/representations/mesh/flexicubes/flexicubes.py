...
try:
    ...
    def check_tensor(tensor, shape, throw=False):
        ok = tensor.dim() == len(shape) and all(expected is None or actual == expected for actual, expected in zip(tensor.shape, shape))
        if not ok and throw:
            raise AssertionError(f"Expected tensor shape {shape}, got {tuple(tensor.shape)}")
        return ok
